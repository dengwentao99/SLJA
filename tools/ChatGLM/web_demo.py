...
model = (
    AutoModel.from_pretrained("THUDM/chatglm-6b", trust_remote_code=True).half().cuda(1)
)
model = model.eval()
...
def predict(input, max_length, top_p, temperature, history=None):
    ...
    for response, history in model.stream_chat(
        ...
        if len(updates) < MAX_BOXES:
            updates = updates + [gr.Textbox.update(visible=False)] * (
                MAX_BOXES - len(updates)
            )
...
with gr.Blocks() as demo:
    ...
    button.click(
        predict, [txt, max_length, top_p, temperature, state], [state] + text_boxes
    )
